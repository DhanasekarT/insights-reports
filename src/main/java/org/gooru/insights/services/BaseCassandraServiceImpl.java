...
public class BaseCassandraServiceImpl implements BaseCassandraService {
	...
	public OperationResult<ColumnList<String>> readColumns(String keyspace, String columnFamilyName, String rowKey, Collection<String> columns) {
		...
		try {
			RowQuery<String, String> rowQuery = getConnector().connectInsights().prepareQuery(accessColumnFamily(columnFamilyName)).setConsistencyLevel(DEFAULT_CONSISTENCY_LEVEL)
	...
	public OperationResult<Rows<String, String>> readAll(String keyspace, String columnFamilyName, Collection<String> keys, Collection<String> columns) {
		...
		try {
			RowSliceQuery<String, String> Query = getConnector().connectInsights().prepareQuery(this.accessColumnFamily(columnFamilyName)).setConsistencyLevel(DEFAULT_CONSISTENCY_LEVEL)
	...
	public ColumnList<String> read(String keyspace, String cfName, String key) {
		...
		try {
			result = getConnector().connectInsights().prepareQuery(this.accessColumnFamily(cfName)).setConsistencyLevel(DEFAULT_CONSISTENCY_LEVEL)
	...
	public void saveStringValue(String keyspace, String cfName, String key, String columnName, String value) {
		...
		MutationBatch m = getConnector().connectInsights().prepareMutationBatch().setConsistencyLevel(DEFAULT_CONSISTENCY_LEVEL).withRetryPolicy(new ConstantBackoff(SLEEP_TIME_MS, LOOP_COUNT));
	...
	public OperationResult<Rows<String, String>> readAll(String keyspace, String columnFamily, Collection<String> columns) {
		...
		try {
			allRowQuery = getConnector().connectInsights().prepareQuery(this.accessColumnFamily(columnFamily)).setConsistencyLevel(DEFAULT_CONSISTENCY_LEVEL).withRetryPolicy(new ConstantBackoff(SLEEP_TIME_MS, LOOP_COUNT)).getAllRows();
			if (!columns.isEmpty()) {
	...
	public BaseConnectionService getConnector() {
		return connector;
	}
